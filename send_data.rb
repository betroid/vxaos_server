...
module Send_Data
	...
	def send_player_state(client, state_id, add_state = false, state_time = 0) #LM²
		buffer = Buffer_Writer.new
		buffer.write_byte(Enums::Packet::PLAYER_STATE)
		buffer.write_short(state_id)
		buffer.write_boolean(add_state)
    buffer.write_float(state_time) if add_state
		client.send_data(buffer.to_s)
	end

	def send_player_buff(client, param_id, buff_level, buff_time = 0, buff_max_time = 0) #LM²
		buffer = Buffer_Writer.new
    ...
    buffer.write_float(buff_time) if buff_level != 0
    buffer.write_float(buff_max_time) if buff_level != 0
		client.send_data(buffer.to_s)
	end
  ...
  #LM² - Cooldown
  def send_player_cooldown(client, id)
    buffer = Buffer_Writer.new
		buffer.write_byte(Enums::Packet::PLAYER_COOLDOWN)
		buffer.write_short(id)
    client.send_data(buffer.to_s)
	end
