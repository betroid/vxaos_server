...
module Game_Character
  ...
  def death_state?
	...
	def add_state(state_id)  #LM²
		return if !state_addable?(state_id)
  ...
  def state_restrict?(state_id)
	...
	def add_new_state(state_id) #LM²
		die if state_id == death_state_id
		@states << state_id unless state?(state_id)
    @states_time[state_id] = (Time.now + $data_states[state_id].min_turns + 0.1) if $data_states[state_id].auto_removal_timing > 0
	...
	def remove_state(state_id) #LM²
		return unless state?(state_id)
		@states.delete(state_id)
    @states_time.delete(state_id) 
	end

	def add_buff(param_id, time) #LM²
		return if death_state?
		@buffs[param_id] += 1 unless buff_max?(param_id)
    @buffs_time[param_id] = Time.now + time + 0.1
		erase_buff(param_id) if debuff?(param_id)
		refresh
	end

	def add_debuff(param_id, time) #LM²
		return if death_state?
  ...
  def update_st_bf_timers #LM² 
    @buffs_time.each_with_index {|timer, param_id|
      next unless timer
      remove_buff(param_id) if Time.now >= timer
    }
    return if @states_time.empty?
    @states_time.each { |state_id, timer|
      remove_state(state_id) if Time.now >= timer
    }
  end
