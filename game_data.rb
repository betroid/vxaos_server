...
module Game_Data
  ...
  def load_game_data
	...
	def load_states
      ...
      #LM² 
      $data_states[state_id].remove_at_battle_end = states[state_id].remove_at_battle_end 
      $data_states[state_id].auto_removal_timing = states[state_id].auto_removal_timing 
      $data_states[state_id].min_turns = states[state_id].min_turns 
      $data_states[state_id].save =  Note.read_boolean('Save', states[state_id].note)
		end
	end
	...
	def load_skills
      ...
      #LM² - Cooldown
      value = Note.read_number('Cooldown', skills[skill_id].note)
      $data_skills[skill_id].cooldown = value && value > 0 ? value : Configs::COOLDOWN_SKILL_TIME
			# Obtém o índice da animação de ataque ou um valor nulo, diferentemente do read_number
			#que retorna 0 se não existir nenhum valor no banco de dados
