...
module Handle_Data
	...
	def handle_chat_message(client, buffer)
		...
		prefix = client.admin? ? '[Admin] ' : client.monitor? ? '[Mod] ' : ''
  ...
  def handle_player_action(client)
    ...
    client.interaction_time = Time.now + Configs::MOUSE_ACTION_TIME
  end
	...
	def handle_use_skill(client, buffer) #LM² - Cooldown
		skill_id = buffer.read_short
		return if client.using_skill?(skill_id)
    ...
    #LM² - Cooldown
		...
		return if (client.hotbar[id].type == Enums::Hotbar::ITEM && client.using_item?)
    return if (client.hotbar[id].type == Enums::Hotbar::SKILL && client.using_skill?(client.hotbar[id].item_id))
