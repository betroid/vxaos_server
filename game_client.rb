...
class Game_Client < EventMachine::Connection
  ...
  #LM² - Cooldown
  def using_skill?(skill_id)
    @skill_cooldown_time.has_key?(skill_id) ? @skill_cooldown_time[skill_id] > Time.now : false
  end
  ...
  def has_seal_equip?(item)
	...
	def add_new_state(state_id, state_time = 0) #LM²
		super(state_id)
    @states_time[state_id] = state_time if state_time != 0
    time = $data_states[state_id].auto_removal_timing > 0 ?  @states_time[state_id] - Time.now : 0
		$network.send_player_state(self, state_id, true, time)
	end
	...
	def add_buff(param_id, time) #LM²
		super
		$network.send_player_buff(self, param_id, 1, @buffs_time[param_id] - Time.now, time)
	end
