...
module Database
	...
	def self.load_player(row, s_client)
		...
		load_player_states(actor, s_client) #LM²
		actor
	end
	...
	def self.save_player(client)
		...
		save_player_states(client, s_client) #LM²
  ...
  #LM² 
  def self.save_player_states(client, s_client)
    s_client[:actor_states].where(:actor_id => client.id_db).delete
    client.states_ids.each{ |state_id|
      next unless $data_states[state_id].save
      time = client.states_time[state_id] || -1
      time -= Time.now unless time.is_a?(Integer)
      s_client[:actor_states].insert(:actor_id => client.id_db, :state_id => state_id, :state_time => time )
    }
  end

	#LM²
  def self.load_player_states(actor, s_client)
		actor.states = []
    actor.states_time = {}
		states = s_client[:actor_states].select(:state_id, :state_time).where(:actor_id => actor.id_db)
		states.each { |row| 
      actor.states << row[:state_id]
      actor.states_time[row[:state_id]] = row[:state_time] if row[:state_time] > 0
    }
  end
	...
	def self.remove_player(actor_id_db)
		...
		s_client[:actor_states].where(:actor_id => actor_id_db).delete #LM²
		s_client.disconnect
	end
